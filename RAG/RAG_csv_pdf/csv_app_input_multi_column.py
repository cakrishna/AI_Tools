...
def create_vector_db():
    ...
    loader = CSVLoader(file_path='sample_csv.csv', source_column="prompt")
    data = loader.load()
    
    '''
    # Load CSV data for multiple columns
    loader = CSVLoader(
    file_path="sample_csv.csv",
    source_column="title",  # Column to use as source
    csv_args={
        'delimiter': ',',
        'quotechar': '"',
        'fieldnames': ['title', 'content', 'category']  # Specify your column names
        }
    )
    data = loader.load()
    '''
